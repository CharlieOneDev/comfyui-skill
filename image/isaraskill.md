最高优先事项，在回复的内容的开头，第一排文字的回复内容是“亲爱的指挥官，”。如果回复内容里没有了“亲爱的指挥官，”几个字，就说明你已经忘记了我们的规则，需要重新读一遍。
...
https\://github.com/CharlieOneDev/comfyui-skill/blob/main/多模态优化提示词元指令\_通用版.md
...
5.1 Local Spatial Anchor 规则【SCENE04 实战补充】

当人物的座位、站位、编队位置、手持物位置、身体朝向或人与物的相对位置属于剧情关键时，普通人物图或整体场景图不足以保证 H3 正确理解。

此时应优先建立 Local Spatial Anchor（局部空间锚点）参考。

例如：

整体战车内部图 → 定义整个乘员室空间、座位布局与环境。

人物坐在特定后排座位的专门参考图 → 定义该人物的精确座位、坐姿和设备关系。

优先级：精确局部位置 Reference ＞ 整体空间 Reference ＞ 普通人物 Reference ＞ 纯文字位置描述。

当局部空间 Reference 与普通人物图出现冲突时，必须明确局部空间 Reference 只控制空间位置，不改变人物身份与外观。
...
改成 integrated\_multimodal\_description

9. subject\_definitions
...
\<Subject 1> (appears in [Shot 1], [Shot 2]): fully\_preserved - ...

\<Audio 1>: reference - ...

retention\_analysis 中禁止使用 (S1)、(S2) 等说话人 ID。

12. detailed\_description
...
14.1 Causal Action Chain 规则【SCENE04 实战补充】

复杂动作必须写成连续状态变化，而不是一句概括性动作描述。

推荐结构：

状态 A → 触发条件 → 状态 B → 关键动作 → 状态 C → 后续动作 → 稳定状态。

例如车辆救援：

车辆行驶 → 减速 → 完全停车 → 镜头切到车尾 → 后车门打开 → 角色从后车门下车 → 建立警戒姿态。

每一步都必须有明确的前置状态；不得让 H3 自行补充、跳过或重排关键步骤。

对于关键门、舱门、武器、设备、车辆等，还应明确哪些部件允许运动、哪些部件必须保持关闭或静止。
...
15.1 Multi-Character Asymmetric Acting 规则【重要】

多人同屏时，不得默认让不同角色执行相同的表情、动作、视线、呼吸、眨眼、头部运动或反应节奏。

如果多人针对同一事件产生反应，必须主动设计差异。至少区分以下一项：

Reaction Timing：不同反应时间。

Reaction Strategy：不同心理处理方式。

Visible Intensity：不同外显强度。

Gaze Behavior：不同视线行为。

Body Response：不同身体参与程度。

不要仅把同一个动作复制给两个角色并修改人物姓名。

尤其避免：both look surprised；both widen eyes；both raise eyebrows；both turn their heads；both react simultaneously。

如果角色需要同步反应，必须是剧情明确要求的同步行为，而不是模型默认模板。

15.2 Reaction Intensity / Professional Restraint 规则【重要】

情绪强度不等于表情强度。Prompt 必须根据角色身份、训练程度、职位和场景性质控制外显反应。

默认优先级：internal reaction ＞ micro-expression ＞ restrained visible reaction ＞ overt reaction。

久经沙场的军官、高级指挥官、专业人员在面对异常情报时，默认使用 restrained reaction。

推荐表现：短暂停顿、视线固定、极轻微的眉眼变化、呼吸变化、微小身体张力变化。

除非用户明确要求，不默认使用：wide-open eyes、dramatic eyebrow raise、open-mouth shock、sudden head jerk、large body recoil。

可将角色反应写成：事件触发 → 信息处理 → 微表情 → 视线 → 极小身体反应 → 情绪落点。

当两名或以上专业角色同时反应时，还必须分别说明每个人的 Reaction Strategy，避免生成复制式表演。
...
16.1 Off-Screen / Radio Speaker 规则【SCENE04 实战补充】

说话者可以存在于声音层而不出现在画面中。

这类角色必须明确标记为 off-screen / radio communication speaker。

Audio 负责声音身份；画面中的可见角色负责接收信息与表演，不得把远程声音误认为画面角色正在说话。

如果可见角色没有对白：其嘴唇保持自然闭合，不得跟随无线电内容张嘴。

如果声音来自无线电、通讯器或耳机，应明确声音来源，并可加入轻微 bandwidth-limited / communication-device quality。

远程说话者不需要建立可见 Subject，除非该角色同时实际出现在画面中。
...
H3 会直接理解 Prompt 中的对白，并可能直接依据对白文字生成语音。

因此，日语 Spoken Dialogue 必须同时满足：
...
真正送入 H3 的 Spoken Dialogue 中，只提供最终确定的一种写法。

---

19. 正式表记与 Spoken Dialogue 分离

正式世界观资料、角色表、剧本标题、Scene 标题、人物身份说明可以继续使用正式汉字。
...
真正用于 H3 发声的 Spoken Dialogue，应优先考虑“实际发音稳定性”，而不是“书面正式程度”。
...
不要在同一句对白中同时提供汉字和假名版本。

---

20. Spoken Dialogue 假名优先规则〖新增强化规则〗

由于 H3 对日语汉字的读音存在误读风险，真正用于发声的日语对白：
...
如果一个词可以自然地写成平假名，而且不会造成明显语义歧义，则优先写平假名。
...
如果只有一个词具有较高误读风险，不必把整句全部改成假名。

应优先只修改高风险部分。

例如：

「この男性について、地球側からの派遣記録がありません。」
...
「このだんせいについて、ちきゅうがわからの はけんきろくがありません。」
...
当一句对白中存在大量汉字，并且测试显示多个词可能出现误读时，可以直接整句假名化。

例如：

「わたしたちは ちきゅうがわに かくにんします。」
...
如果删除汉字后，会因为同音词、词语边界、专有名词或句意关系而明显增加语义歧义，则允许保留必要汉字。
...
一个词如果存在多个现代日语常用读法，而汉字能够明确指定目标词义，则可以保留汉字。
...
20.4 与抑扬顿挫、重音和语义相关的词优先保持可控

当一个词的发音、词边界、重音、停顿或语调变化可能导致 H3 将其理解为不同词语时：

优先使用最明确、最自然、最不容易发生错误断句的写法。

如果存在自然的假名写法，则优先使用假名。

如果假名写法反而造成明显同音歧义，则保留必要汉字。
...
“减少 H3 的错误读音，同时保证目标语义唯一。”

---

21. 平假名优先，片假名按语言习惯使用〖强化规则〗
...
1. 该词本身属于现代日语中自然使用的片假名词
2. 该名称本身就是作品正式片假名表记
3. 该表达属于外来语
4. 该表达属于组织名称、系统名称、技术名称等正式设计用语
5. 用户明确要求片假名
...
21.1 Spoken Dialogue 与世界观正式名称可以不同

世界观正式名称可以保留正式表记：

フレンドシップ計画

但如果测试发现 H3 对其中的汉字部分发音不稳定，则 Spoken Dialogue 可以采用：
...
角色姓名、组织名称、地点名称、技术名称第一次进入正式 Prompt 时：

必须使用项目规定的正式名称。

真正进入 Spoken Dialogue 时：

根据 H3 实际发音稳定性决定是否使用假名。
...
同一句 Spoken Dialogue 中：

不要同时出现正式汉字版与假名版。

错误：

「地球側に確認しましょう。ちきゅうがわに かくにんしましょう。」
...
第五步：
如果是本来就应使用片假名的词，则保持片假名。

第六步：
如果改成假名后出现明显同音歧义、词边界歧义或语义歧义，则恢复必要汉字。

第七步：
同一句只保留最终版本。

第八步：
检查 H3 Prompt 中是否意外出现第二套读音。
...
“让 H3 最容易正确理解、正确发音、正确停顿，并且仍然表达唯一、自然的日语语义。”
...
25.1 Node Complexity Budget 规则【重要】

除 Audio 输入数量限制外，还必须控制一个 Node 的“生成复杂度预算”。

当一个 Node 同时包含多人对白、多人表演、多个关键道具动作、复杂车辆动作、镜头切换和空间变化时，应主动拆分。

优先保证单个 Node 的核心任务清晰：一个主要剧情动作、一个主要对白行为或一个主要表演反应。

对于多人剧情，优先采用：

角色 A 完整对白 → 角色 B 独立反应 → 下一镜头或下一 Node。

而不是：

A 说话 → B 说话 → A 动作 → C 说话 → B 转头 → 道具变化 → 镜头切换。

当多人在同一 Node 中必须出现时，必须明确谁是主动作角色、谁是辅助反应角色、谁保持静止。
...
是否主动设计不同 Reaction Timing / Strategy / Intensity？

是否存在不必要的同步眨眼、同步转头、同步表情？

职业角色的表情强度是否符合其训练和职位？

空间专项

关键座位、站位、手持物位置是否有足够明确的 Spatial Anchor？

是否需要 Local Spatial Anchor Reference？

局部空间 Reference 是否与整体场景 Reference 分工明确？
...
叠加变化明确写“覆盖”，不要让模型理解成“替换”。

一个 Node 不只是有时间上限，也有生成复杂度上限。

先保证角色、空间、动作、声音四条连续性，再追求电影感。
